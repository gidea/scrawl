...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  if (command !== 'start') {
    return { command: 'help', options: { workspaceId: '', configPath: '.scrawl/config.json' } };
  }

  const options: StartCommandOptions = {
    workspaceId: '',
    configPath: '.scrawl/config.json',
  };
...
function printUsage(): void {
  process.stderr.write(`Usage: scrawl-run start --workspace <id> [--config path]\n`);
}
