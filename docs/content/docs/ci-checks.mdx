---
title: CI/CD Checks
description: Monitor GitHub Actions check runs directly in Scrawl
---

Scrawl surfaces your GitHub Actions CI/CD status inside the app so you can monitor check runs without switching to a browser.

<img
  src="/media/cicd.png"
  alt="CI/CD checks panel in Scrawl"
  style={{
    width: '100%',
    maxWidth: '700px',
    margin: '20px auto',
...
Scrawl automatically polls for check updates:

- **Every 10 seconds** while checks are still running
- **Every 60 seconds** once all checks complete
- **On window focus** — checks refresh when you switch back to Scrawl
