...
MCP (Model Context Protocol) lets your agents connect to external tools and data sources — databases, APIs, browsers, and more. Scrawl manages MCP server configs across all your installed agents from a single place, so you configure once and every agent picks it up.
...
Scrawl writes the config into each selected agent's native config file. The server is available in your next agent session.
...
When you add or edit a server, Scrawl translates the config into each agent's native format and writes it to the correct location:
...
This means MCP servers configured through Scrawl also work when you run these agents outside of Scrawl.
...
MCP servers enable Scrawl to connect to cloud vector databases for retrieval-augmented generation (RAG). This is a key part of Scrawl's vision for research-backed writing workflows — connecting agents to your knowledge base so they can produce better-informed content. Stay tuned for built-in vector database MCP server support.
...
- Click **Refresh** to re-detect installed agents if you've installed a new CLI since opening Scrawl.
