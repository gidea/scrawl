{
  description = "Nix dev shell for the Scrawl Electron workspace";
          ...
          if pkgs.stdenv.isLinux then
            pkgs.stdenv.mkDerivation rec {
              ...
              installPhase = ''
                ...
                install -d $out/share/scrawl
                cp -R "$unpackedDir" $out/share/scrawl/

                if ls "$distDir"/*.AppImage >/dev/null 2>&1; then
                  for image in "$distDir"/*.AppImage; do
                    install -Dm755 "$image" "$out/share/scrawl/$(basename "$image")"
                  done
                fi

                install -d $out/bin
                cat <<EOF > $out/bin/scrawl
#!${pkgs.bash}/bin/bash
set -euo pipefail

APP_ROOT="$out/share/scrawl/linux-unpacked"
exec "\$APP_ROOT/scrawl" "\$@"
EOF
                chmod +x $out/bin/scrawl

                runHook postInstall
              '';

              meta = {
                description = "Scrawl – multi-agent orchestration desktop app";
                homepage = "https://scrawl.sh";
                license = lib.licenses.mit;
                platforms = [ "x86_64-linux" ];
              };
            }
          else
            pkgs.writeShellScriptBin "scrawl" ''
              echo "The packaged Scrawl app is currently only available for Linux when using Nix." >&2
              exit 1
            '';
      in {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            echo "Scrawl dev shell ready"
            echo "Node: $(node --version)"
            echo "Run 'pnpm run d' for the full dev loop."
          '';
        };

        packages.scrawl = scrawlPackage;
        packages.default = scrawlPackage;

        apps.default = {
          type = "app";
          program = "${scrawlPackage}/bin/scrawl";
        };
      });
}
